...
## Make prediction
Since in previous steps, we only work with real number, in this step, we must convert numbers into integer and then
...
```{r eval=FALSE}
predictions <- round(predict(modelFit, newdata=training))
sum(predictions == training$classe) / length(predictions)
predictions <- round(predict(modelFit, newdata=cv))
sum(predictions == cv$classe) / length(predictions)
```

The prediction accuracy on the training dataset and Cross-validation data set are 79.63% and 72.37%, respectively.
I would expect the accuracy on the test dataset to be around the same (70-80%).
